...
$ch="select * from login where status='Not_verified' and email='$uname'";
...
if(mysqli_fetch_array($res))
{
    ...
    echo "<script>Swal.fire({
        title: 'Dear User?',
        text: 'You are not a verified user now!Please verify your Email address in next step!',
        icon: 'warning',
...
{
  $chck="select status from userdetails where email='$uname'";
...
if($s=="0")
  ...
  {  
$check="select * from login where email='$uname' and password='$password'";
$result=mysqli_query($con,$check) or die("query failed".mysqli_error($con));
if(mysqli_fetch_array($result))
{
$_SESSION['users']=$uname;
header('Location:../indexjobseeker.php');
}
else
{
echo "<script>Swal.fire({
